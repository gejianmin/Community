...
- (void)configCameraAndStart{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        self.dataOutput.metadataObjectTypes = @[AVMetadataObjectTypeQRCode,//这个是二维码
                                                //以下八种类型是条形码
...
- (CGRect)scanRect{
    if (CGRectIsEmpty(_scanRect)) {
        ...
        _scanRect = CGRectMake((self.frame.size.width - scanSize.width)/2, (self.frame.size.height/2 - scanSize.height/2-44), scanSize.width, scanSize.height);
    }
    return _scanRect;
}
