...
@interface RegisterView ()<HHCaptchaButtonDelegate>
...
-(void)getValidateButtonAction:(HHCaptchaButton *)sender{
    if ([self .delegate respondsToSelector:@selector(registerViewGetVFWithButton:phoneNum:)]) {
        [self.delegate registerViewGetVFWithButton:sender phoneNum:self.phoneNumField.text];
    }
    
    
}
-(void)loginEvent:(UIButton *)sender{
        if ([self.delegate respondsToSelector:@selector(registerViewDidLoginWithPhoneNum:VFNum:password:rePassword:)]) {
            [self.delegate registerViewDidLoginWithPhoneNum:self.phoneNumField.text VFNum:self.vfField.text password:self.psdField.text rePassword:self.repsdField.text];
        }
}
...
-(CustomBtn *)postBtn{
    ...
    if (!_postBtn) {
        _postBtn = [[CustomBtn alloc]initWithFrame:CGRectZero Tag:1000 Title:@"立即登录" backgroundColor:kColorRed TitleTextColor:kColorWhite Font:17 Image:nil];
