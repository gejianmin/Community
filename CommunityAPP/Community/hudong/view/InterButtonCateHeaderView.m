...
@implementation InterButtonCateHeaderView
...
-(void)setupUI{
    ...
    [self.leftLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.iconIma.mas_top);
    }];
...
-(void)setModel:(Inter_categaryModel *)model{
    self.leftLabel.text = model.topic_name;
    self.nickLabel.text = [NSString stringWithFormat:@"%@ 人关注",model.follow_number];
    self.VIPLabel.text = model.topic_desc;
    [self.iconIma sd_setImageWithURL:[NSURL URLWithString:model.topic_logo] placeholderImage:ImageNamed(@"icon")];
}
