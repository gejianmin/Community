...
//    UIPanGestureRecognizer *panPG = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(panPressAction:)];
...
- (void)longPressAction:(UILongPressGestureRecognizer *)longPG{
    if (self.imageBtns_MARR.count) {
        ...
        CGPoint point = [longPG locationInView:self];//当前触摸点的坐标
//        CGPoint ve_point = [longPG velocityInView:self];
//        NSLog(@"ve_point = %f",ve_point.x);
...
- (void)panPressAction:(UIPanGestureRecognizer *)longPG
{
    if (self.imageBtns_MARR.count) {
...
- (void)loadingImageView
{
    ...
    if (self.images_MARR.count < self.maxItem) {
        ...
        [plus_BTN setBackgroundImage:[UIImage imageNamed:@"PostCircleAddPicBtnImage"] forState:UIControlStateNormal];
