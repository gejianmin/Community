...
-(NSString *)substringFromIndexSafe:(NSUInteger)from{
    ...
    if (start<0) {
        DLog(@"NSString substringFromIndexSafe from %lu out of bounds,less than 0,string=%@",(long)from,self);
        return [self copy];
    }
    if (start>self.length) {
        DLog(@"NSString substringFromIndexSafe from %lu out of bounds,large than string length %lu,string=%@",(unsigned long)from,(unsigned long)self.length,self);
        return nil;
    }
...
-(NSString *)substringToIndexSafe:(NSUInteger)to{
    NSInteger end=to;
    if (end<0) {
        DLog(@"NSString substringToIndexSafe end %ld out of bounds,less than 0,string=%@",(long)end,self);
        return nil;
    }
    if (end>=self.length) {
        DLog(@"NSString substringToIndexSafe end %lu out of bounds,large than string length %lu,string=%@",(unsigned long)to,(unsigned long)self.length,self);
        return [self copy];
    }
...
-(NSString *)substringWithRangeSafe:(NSRange)range{
    ...
    if (range.length==0) {//这种情况对API合法，但是开发中一般不可能使用
        DLog(@"NSString substringWithRangeSafe subRange length 0 of out of range (%d,%lu),string=%@",0,(unsigned long)self.length,self);
    }
#endif
    if (range.location>self.length) {
        DLog(@"NSString substringWithRangeSafe range out of bounds,rang (%lu,%lu) out of range (%d,%lu),string=%@",(unsigned long)range.location,(unsigned long)range.length,0,(unsigned long)self.length,self);
        return nil;
    }
...
#endif
    if (range.location>self.length) {
        DLog(@"NSString stringByReplacingCharactersInRangeSafe range out of bounds,rang (%lu,%lu) out of range (%d,%lu),string=%@",(unsigned long)range.location,(unsigned long)range.length,0,(unsigned long)self.length,self);
        return nil;
    }
    if (range.location+range.length>self.length) {
        DLog(@"NSString stringByReplacingCharactersInRangeSafe range out of bounds,rang (%lu,%lu) out of range (%d,%lu),string=%@",(unsigned long)range.location,(unsigned long)range.length,0,(unsigned long)self.length,self);
