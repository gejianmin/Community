...
-(void)reloadRowsAtIndexPathsSafe:(NSArray *)indexPaths withRowAnimation:(UITableViewRowAnimation)animation{
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        ...
        if(indexPath.section<0||indexPath.section>=totalSelections||indexPath.row>=totalRows){
            DLog(@"UITableView reloadRowsAtIndexPathsSafe find indexPath %@ out of bounds ,tableView cell class type=%@",indexPath,[[self visibleCells]count]>0?[[[self visibleCells] objectAtIndex:0] class]:@"unknown");
        }else{
