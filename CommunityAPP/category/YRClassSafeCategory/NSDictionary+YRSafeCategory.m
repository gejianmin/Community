...
+(id)dictionaryWithObjectSafe:(id)object forKey:(id<NSCopying>)key{
    if (!key||!object) {
        DLog(@"NSDictionary dictionaryWithObjectSafe set nil value or key ,value=%@,key=%@",object,key);
        return nil;
    }
...
-(id)objectForKeySafe:(id)aKey{
    if (!aKey) {
        DLog(@"NSDictionary objectForKeySafe use nil key,dictionary=%@",self);
        return nil;
    }
...
-(void)setObjectSafe:(id)anObject forKey:(id<NSCopying>)aKey{
    if (!aKey||!anObject) {
        DLog(@"NSDictionary setObjectSafe set nil value or key ,value=%@,key=%@,dictionary=%@",anObject,aKey,self);
        return;
    }
