...
- (void)setCameraRollAtFirst
{
    for (ALAssetsGroup *group in _assetGroups)
    {
        if ([[group valueForProperty:@"ALAssetsGroupPropertyType"] intValue] == ALAssetsGroupSavedPhotos)
        {
            ...
            [_assetGroups insertObject:self.group atIndex:0];
            return;
        }
    }
}
- (void)getGroupVideosList:(void (^)(NSArray *))result{
    [self initAsset];
...
- (void)getVideoListOfGroup:(ALAssetsGroup *)alGroup result:(void (^)(NSArray *))result
{
    [self initAsset];
    
    _assetPhotos = [[NSMutableArray alloc] init];
    [alGroup setAssetsFilter:[ALAssetsFilter allVideos]];
    [alGroup enumerateAssetsUsingBlock:^(ALAsset *alPhoto, NSUInteger index, BOOL *stop) {
        
        if(alPhoto == nil)
        {
            if (_bReverse)
                _assetVideos = [[NSMutableArray alloc] initWithArray:[[_assetVideos reverseObjectEnumerator] allObjects]];
            
            result(_assetVideos);
            return;
        }
        
        [_assetVideos addObject:alPhoto];
    }];
}
...
- (NSInteger)getVideoCountOfCurrentGroup{
    return _assetVideos.count;
}
