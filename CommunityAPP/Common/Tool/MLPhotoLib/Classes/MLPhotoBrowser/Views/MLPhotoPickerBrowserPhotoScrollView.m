//
//  MLPhotoPickerBrowserPhotoScrollView.m
//  MLPhotoLib
//
...
@interface MLPhotoPickerBrowserPhotoScrollView ()<UIActionSheetDelegate,UIScrollViewDelegate> {
    MLPhotoPickerBrowserPhotoView *_tapView; // for background taps
}
...
@implementation MLPhotoPickerBrowserPhotoScrollView
...
- (id)initWithCoder:(NSCoder *)coder{
    if ((self = [super initWithCoder:coder])) {
        ...
        _tapView = [[MLPhotoPickerBrowserPhotoView alloc] initWithFrame:self.bounds];
        ...
        // Image view
        _photoImageView = [[MLPhotoPickerBrowserPhotoImageView alloc] initWithFrame:CGRectZero];
        ...
        progressView.frame = CGRectMake(0, 0, MLPickerProgressViewW, MLPickerProgressViewH);
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(long)buttonIndex{
    if (buttonIndex == 0){
        if([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeSavedPhotosAlbum]) {
            ...
            UIImage *editedImage = _photoImageView.image;
            if (gtiOS8) {
                [MLPhotoKitData addAssetAlbumForName:@"Camera Roll" image:editedImage completionHandler:^(BOOL success, NSError * _Nullable error) {
...
- (void)setPhoto:(MLPhoto *)photo{
    ...
    if (photo.assetUrl.absoluteString.length ||
        ...
        // 本地相册
        NSRange photoRange = [photo.assetUrl.absoluteString rangeOfString:@"assets-library"];
        if (photoRange.location != NSNotFound && photoRange.length > 0){
        ...
        } else if (photo.origianlImage != nil) {
            _photoImageView.image = photo.origianlImage;
            self.isLoadingDone = YES;
            [self displayImage];
        } else{
            ...
            _photoImageView.frame = [MLPhotoRect setMaxMinZoomScalesForCurrentBoundWithImageView:_photoImageView];
            
            if (_photoImageView.image == nil) {
                [self setProgress:0.01];
            }
            
            [NSURLConnection sendAsynchronousRequest:[NSURLRequest requestWithURL:photo.originalImageUrl?:photo.thumbImageUrl] queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse * _Nullable response, NSData * _Nullable data, NSError * _Nullable connectionError) {
    ...
    } else if (photo.origianlImage){
        _photoImageView.image = photo.origianlImage;
        self.isLoadingDone = YES;
        [self displayImage];
    } else if (photo.thumbImage){
        photo.origianlImage = photo.thumbImage;
