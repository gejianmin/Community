...
@dynamic borderColor;
@dynamic cornerRadiusOfBottom;
@dynamic cornerRadiusOfBottomLeft;
@dynamic cornerRadiusOfBottomRight;
@dynamic cornerRadiusOfTop;
@dynamic cornerRadiusOfTopLeft;
@dynamic cornerRadiusOfTopRight;
...
//    [self addSubview:line];
    [self addBottomLine:h color:kLine_Color];

}
-(void)addBottomLine:(CGFloat)h color:(UIColor *)color {
    
    UIView *line=[[UIView alloc]initWithFrame:CGRectMake(0, self.height-h, self.width, h) color:color];
    [self addSubview:line];
}
...
-(void)addLeftLine:(CGFloat)w color:(UIColor *)color {
    
    UIView *line=[[UIView alloc]initWithFrame:CGRectMake(0, 0, w, self.height) color:color];
    [self addSubview:line];
}
...
-(void)addRightLine:(CGFloat)w space:(CGFloat)space{
    UIView *line=[[UIView alloc]initWithFrame:CGRectMake(self.width-w, space, w, self.height - space * 2) color:kLine_Color];
    [self addSubview:line];
...
- (void)configCornerOfCircle {
    self.layer.masksToBounds = YES;
    self.layer.cornerRadius = CGRectGetWidth(self.bounds) * .5f;
    self.layer.borderWidth = .5f;
    self.layer.borderColor = [UIColor clearColor].CGColor;
...
- (void)setVertifyType:(UITextFieldVertifyType)vertifyType {
    objc_setAssociatedObject(self, @selector(vertifyType), @(vertifyType), OBJC_ASSOCIATION_ASSIGN);
    [self removeTarget:self action:@selector(limitMaxInputTextLength:) forControlEvents:UIControlEventEditingChanged];
    [self addTarget:self action:@selector(limitMaxInputTextLength:) forControlEvents:UIControlEventEditingChanged];
}
- (UITextFieldVertifyType)vertifyType{
    return  [objc_getAssociatedObject(self, @selector(vertifyType)) integerValue];
}

- (void)limitMaxInputTextLength:(UITextField *)sender{
    
    NSString *temp=sender.text;
    if(self.vertifyType == UITextFieldVertifyTypeOnlyWordAndNumber) {
        if (self.markedTextRange==nil) {
            while (1) {
                
                if ([self inputShouldLetterOrNum:temp]) {
                    break;
                }else{
                    if(temp.length == 0) break;
                    temp=[temp substringToIndex:temp.length-1];
                    [self routerEventWithName:@"1231231231231231" userInfo:@{@"textField":self}];
                }
