...
+(NSString *)stringWithCurString:(NSString *)curString range:(NSRange)range replaceString:(NSString *)replaceString{
    ...
    NSMutableString *numString = [[NSMutableString  alloc] initWithString:curString];
    [numString replaceCharactersInRange:range withString:replaceString];
    return numString;
}
+(NSString *)timeStamp
{
...
+(NSString *)dateFmtStr:(NSString *)fmt date:(NSDate *)date{
    ...
    NSDateFormatter *dfmt=[[NSDateFormatter alloc]init];
    [dfmt setDateFormat:fmt];
    NSString *str=[[dfmt stringFromDate:date] copy];
    return str;
}
...
+ (NSString *)getHandlingTimeString:(NSString *)date{
    ...
    NSDate *data = [dateFormatter dateFromString:date];
    [dateFormatter setDateFormat:@"yyyy.MM.dd'T'hh:mm:ss"];
    
    NSString *timeS = [dateFormatter stringFromDate:data];
    
    NSRange range = [timeS rangeOfString:@"T"];
    
    NSString *newTime = [timeS substringToIndex:range.location];
...
+(NSString *)stringTrasformtoDateString:(NSString *)dataStr{
    ...
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:[dataStr integerValue]/1000];
...
+(NSArray *)stringTrasformtoArray:(NSString *)arrayString{
    ...
    NSString * imagesString = [arrayString stringByReplacingOccurrencesOfString:@" " withString:@""];
    NSArray  *array = [imagesString componentsSeparatedByString:@","];
    return array;
}
