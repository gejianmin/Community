...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.delegate = self;
        self.maxNumber = 100;//初始化最大文字数量，默认100个
        [self setup];
    }
    return self;
}
...
- (void)textDidChange
{
    if (self.text.length > self.maxNumber
        ) {
        [self setText:[self.text substringToIndex:self.maxNumber]];
    }
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if (range.location + text.length > self.maxNumber
        ) {
//        return NO;
        //字数限制操作
